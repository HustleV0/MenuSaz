...
const quickActions = [
  ...
  },
  {
    ...
    href: `/preview/${mockRestaurant.slug}`,
    icon: Eye,
    external: true,
  },
  {
    ...
    action: () => {
      if (typeof window !== 'undefined') {
        const url = `${window.location.origin}/preview/${mockRestaurant.slug}`;
...
export default function DashboardPage() {
  ...
  const menuUrl = mounted ? `${window.location.origin}/preview/${mockRestaurant.slug}` : '';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">سلام، {mockRestaurant.name}!</h1>
          <p className="text-muted-foreground">
            خلاصه‌ای از وضعیت وبسایت شما
          </p>
        </div>
        <div className="flex items-center gap-2">
           <Link href={`/preview/${mockRestaurant.slug}`} target="_blank">
